...
export default class ButtonInterpreter {
  ...
  private parseEventType(state: SingleButtonState, duration_ms: number): Event {
    switch (state) {
      case 'PRESSED':
   ...
   * @private
   */
  private getButtonSequence(button_log: ButtonLog[]): Event[] {
    return button_log.reduce<Event[]>((button_events: Event[], current: ButtonLog, index: number, commands: ButtonLog[]) => {
